...
- [ ] 유저 정보 조회
  - [ ] `유저 정보 조회` API가 있다는 건, 유저 정보를 가져오기 위해 localStorage 등에 `userId` 만 저장해두었다가 조회하라는 의미일까?
  - [ ] 그렇게 되면 `accessToken` 은 사용되지 않아 의미 없는 데이터가 된다.
  - [ ] 새로고침 시에 localStorage 등에 저장된 `accessToken`을 통해 유저 정보 조회를 하는 걸 생각했으나 API를 보면 아닌 것 같다
  - [ ] 일단은 응답 전체를 localStorage에 저장하는 방식으로 진행하자.
...
- [ ] 성능을 고려하여 작업해주세요.
- [ ] 한 페이지 당 보여지는 상품의 개수는 16개입니다.
- [ ] 상품을 클릭하면 상품 상세(`/products/{id}`)로 이동합니다.
- [ ] 상품 가격은 세 자리마다 콤마(,)로 구분해주세요.
- [ ] 더 이상 가져올 데이터가 없는 경우 요청이 나가지 않도록 처리해주세요.
- [ ] 상품 상세 화면으로 이동했다가 다시 이전 페이지(`/infinite-scroll`)로 돌아오면 기존의 위치로 되돌아옵니다.
